...
class RPS_Admin_Menu_Department extends RPS_Admin_Menu_MenuAbstract {
        ...
        <script type="application/javascript">
            jQuery(function($){
                ...
                $('.delete_department').click(function() {
                    ...
                    var str = 'Are you sure you want to delete branch: <strong>' + th.data('department_name') +'</strong> ?'
    ...
    private function add_new() {
        ...
        if ( ! empty( $_POST ) && check_admin_referer( 'create_department_nonce', 'create_department' ) ) {
            ...
            // Error checking
            if( !$name ) {
                $this->error['name'] = __("Please specify a Branch name.", $this->TD);
            } elseif($this->getDepartmentByName($name) != NULL) {
                //check branch name exist in database
                $this->error['name'] = __("Branch name already exists in database.", $this->TD);
            }
            if( !$slug ) {
                $this->error['slug'] = __("Please specify a Branch slug.", $this->TD);
            }
            else {
                if ( strlen( $slug ) >= 28 ) {
            ...
            if( !$full_name ) {
                //$this->error['full_name'] = __("Please specify Branch full name.", $this->TD);
            }
            ...
            if (empty($this->error ) && empty($this->messages)) {        
                ...
                if($wpdb->insert( $wpdb->rps_department, $data , $format)) {
                    ...
                    $this->messages[] = __("Branch Added Successfully...", $this->TD);
        ...
        if ( ! empty( $_POST ) && check_admin_referer( 'edit_department_nonce_' . $department_id, 'edit_department' ) ) {
            ...
            if(empty($this->error)):
                ...
                if($wpdb->update($wpdb->rps_department,$data,$where,$format,$format_where)) {
                    $this->messages[] = __("Branch Updated Successfully.",$this->TD);
                    $data = array();
    ...
    private function delete () {
        ...
        if ( current_user_can($role) && isset($_GET['delete_department']) && wp_verify_nonce($_GET['delete_department'], 'delete_department_' . $department_id)) {
            ...
            if ( is_array($row) && !empty($row) && intval( $row['cnt'] ) != 0) {
                $error = true;
                $this->errors[] = __('You can\'t delete this Branch. Branch is assigned to batch. Please delete batch first to delete this Branch', $this->TD);
            }

            //check Branch id is used with any student
            if ( !$error ) {
	            ...
	            if ( is_array( $result ) && ! empty( $result ) ) {
		            $error          = true;
		            $this->errors[] = __( 'You can\'t delete this Branch. Branch is assigned to Students. Please delete assigned Students or reassign Students to another Branch first to delete this Branch', $this->TD );
	            }
            }

            //check Branch id is used with any course
            if ( !$error ) {
	            $query_args['meta_query'][] = array(
		            'key'     => '_department_id',
		            'value'   => $department_id,
	            ...
	            if ( is_array( $result ) && ! empty( $result ) ) {
		            $error          = true;
		            $this->errors[] = __( 'You can\'t delete this Branch. Branch is assigned to Courses. Please delete assigned Courses or reassign Courses to another Branch first to delete this Branch', $this->TD );
	            }
            }

	        //check Branch id is used with any result
            if ( !$error ) {
	            $query  = $wpdb->prepare( "SELECT id FROM {$wpdb->rps_exam_record} WHERE department_id = %d LIMIT 1", array( $department_id ) );
	            $result = $wpdb->get_row( $query, ARRAY_A );
	            if ( is_array( $result ) && ! empty( $result ) ) {
		            $error          = true;
		            $this->errors[] = __( 'You can\'t delete this Branch. Branch is assigned to Exams. Please delete assigned Exams or reassign Exams to another Branch first to delete this Branch', $this->TD );
	            }
            }
            ...
            if ( !$error ) {
	            ...
	            if ( is_array($row) && !empty($row) ) {
		            //drop taxonomy table, ie: delete all the semester associated with this Branch
		            ...
		            $this->messages[] = __('Branch deleted successfully.', $this->TD);
    ...
    ?>
        ...
        <div id="col-left">
            <div class='col-wrap'>
                <div class='form-wrap'>
                    <h3><?php _e( 'Add New Institute Branch', $this->TD ) ?></h3>
                    <form method="post" action="">
                        <?php echo $nonce; echo $hidden_field; ?>

                        <div class="form-field form-required">
                            <label for="name"><?php _e( 'Branch Name', $this->TD) ?> (*)</label>
                            <input name="name" id="name" type="text" size="40" aria-required="true" value="<?php echo $data['name'];   ?>" />
                            <p><?php _e( 'The short name of branch, use Uppercase letters with no space. eg CSE', $this->TD ) ?></p>
                            <?php if(isset($this->error['name'])) echo '<p style="color:red;"><strong>' . $this->error['name'] . '</strong></p>';  ?>
                        </div>
                
                        <div class="form-field">
                            <label for="slug"><?php _e('Branch Name Slug', $this->TD ) ?> (*)</label>
                            <input name="slug" id="slug" type="text" size="40" aria-required="false" value="<?php echo $data['slug'];   ?>" <?php echo $readonly; ?> />
                            <p><?php _e('The "slug" is the URL-friendly version of the name. It is usually all lowercase and contains only letters, numbers, and underscore. <strong>Leave blank for lowercase version of your Branch name.</strong> eg. cse', $this->TD ) ?></p>
                        ...
                        <div class="form-field form-required">
                            ...
                            <p><?php _e('eg. Kailash Nagar', $this->TD) ?></p>
                        ...
                        <div class="form-field">
                            ...
                            <p><?php _e('Write a short description about branch.', $this->TD) ?></p>
                        </div>
                        ...
                        <div class="form-field form-required">
                            ...
                            <p><?php _e('Enable this branch?', $this->TD) ?></p>
                            <?php if(isset($this->error['active'])) echo '<p style="color:red;"><strong>' . $this->error['active'] . '</strong></p>';  ?>
                        </div>
                
                        <p class="submit">
                            <input type="submit" name="submit" id="submit" class="button button-primary" value="<?php if(isset($_REQUEST['edit'])) { echo __("Edit Branch", $this->TD); } else { echo  __("Add New Branch", $this->TD);} ?>"/>
            ...
            <div class="col-wrap">
                <table class="wp-list-table widefat fixed striped tags" style="width: 100%;">
                    ...
                    <tbody>
                        ...
                        ?>
                        <tr><td colspan="4"><?php _e('No Branch Found.', $this->TD) ?></td></tr>
                        <?php
                            }
                            else {
    ...
    private function get_header() {
        ...
        $department_header_h2 .= __('Branch List', $this->TD);
