...
$output  = '<!-- rt-portfolio-box-filter --><div class="rt-portfolio-box-filter element-one filter-style-' . esc_attr( $shortcode['portfolio_filter_style'] ) . ' text-' . esc_attr( $shortcode['portfolio_filter_alignment'] ) . ' ' . esc_attr( $hidden_filter ) . '">';
...
foreach ( $cats as $cat ) {
	...
	$output .= '<!-- button --><button class="matchHeight" data-filter=".';
	$output .= $ptype_slug;
	$output .= '"><span>';
	$output .= $ptype_name;
	$output .= '</span></button><!-- button -->';
}

$output .= '<!-- button --><button class="matchHeight current-menu-item" data-filter="*"><span>Show All</span></button><!-- button -->';
$output .= '</div><!-- rt-portfolio-box-filter -->';
$output .= wp_reset_query();
$output .= '<!-- rt-portfolio-box --><div class="rt-portfolio-box element-one row isotope ' . esc_attr( $enable_zoom ) . '" data-portfolio-box-align="' . esc_attr( $shortcode['portfolio_box_alignment'] ) . '" style="margin-left:-' . esc_attr( $spacing_value ) . 'px; margin-right:-' . esc_attr( $spacing_value ) . 'px;">';
...
if ( $my_query->have_posts() ) {
	while ( $my_query->have_posts() ) :
		...
		$output .= '<!-- rt-portfolio-box-item --><div data-wow-delay="0.3s" class="rt-portfolio-box-item '. $rt_animation .' ';
		foreach ( $terms as $term ) {
			$output .= $term->slug . ' ';
		}
		$output .= $portfolio_item_class . '"  '.$time.' style="padding:' . esc_attr( $spacing_value ) . 'px;">';
    		$output .= '<div class="holder">';
    		    $output .= '<!-- pic --><div class="pic">';
    		    ...
    		    $output .= '</div><!-- data -->';
    		$output .= '</div>';
		$output .= '</div><!-- rt-portfolio-box-item -->';
	endwhile;
}
